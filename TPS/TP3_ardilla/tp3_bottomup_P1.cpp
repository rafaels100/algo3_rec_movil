...
void completarDPT(int salto){
  //para los primeros h = 0 calculo la solucion optima a mano
  for (int k = 0; k < m; k++) DPT[0][k] = nueces[0][k];
  //para las siguientes alturas, antes de tener la posibilidad de dar el salto,
  //los caminos optimos van a ser tomar la nuez actual mas el optimo de bajar
  for (int i = 1; i < salto; i++) {
    for (int k = 0; k < m; k++) DPT[i][k] = DPT[i-1][k] + nueces[i][k];
  }
  ...
  for (int i = salto; i < n; i++) {
    for (int j = 0; j < m; j++) {
      if ((j + 1 < m) && (j - 1 >= 0)) {
        ...
        int max_izq_der = max(DPT[i-salto][j+1], DPT[i-salto][j-1]);
        ...
        DPT[i][j] = nueces[i][j] + max(DPT[i-1][j], DPT[i-salto][j+1]);
      }else{
